...
export const AreaHubPage: FC<{ areaSlug: string }> = ({ areaSlug }) => {
  ...
  const hubFaqs = [
    ...
    { q: `${area.name}에서 어떤 진료를 받을 수 있나요?`, a: `${CLINIC.name}는 임플란트, 투명교정, 미니쉬 심미치료, 치아교정을 포함한 통합치의학과 전 과목 진료를 제공합니다. ${area.name} 지역 환자분이 한 곳에서 모든 진료를 받으실 수 있습니다.` },
  ...
  return (
    <Layout
      ...
      description={`${area.fullName} 인근 치과 ${CLINIC.name}. ${area.distance || ''} 임플란트·투명교정·미니쉬·치아교정 통합 진료. ${area.intro?.slice(0, 60) || ''}`}
      ...
      <section class="sec">
        <div class="container article-body">
          ...
          <p class="aeo-answer">
            ...
            통합치의학과 전문의가 임플란트·투명교정·미니쉬·치아교정을 정밀하게 진료합니다.
          </p>
          {area.intro && <p>{area.intro}</p>}
